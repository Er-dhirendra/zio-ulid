 ...
 *  - '''Monotonic''': Within the same millisecond, the random component
 *    is incremented by 1, guaranteeing strict lexicographic ordering even
 *    at high throughput.
 *
 * Usage:
 * {{{
...
object ULIDGen {
  ...
  /**
   * Monotonic live layer: guarantees ordering within the same millisecond.
   * Recommended for database IDs and any use case requiring strict ordering.
   */
  val monotonic: ULayer[ULIDGen] =
    ZLayer.fromZIO(makeMonotonic)

  /**
   * Fast layer: uses [[java.util.concurrent.ThreadLocalRandom]] — not
   * cryptographically secure. Suitable for logging and tracing.
   */
   ...
   *
   * Within a single millisecond the random component is incremented by 1
   * instead of being re-randomised, guaranteeing that every generated ULID
  ...
  /**
   * Creates a deterministic [[ULIDGen]] for testing that generates ULIDs
   * from a fixed sequence starting at the given time with zero random bits,
